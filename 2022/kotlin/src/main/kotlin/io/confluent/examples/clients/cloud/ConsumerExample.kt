...
fun main(args: Array<String>) {
    ...
    consumer.use {
        while (true) {
            totalCount = consumer
                    .poll(ofMillis(100))
                    // Use fold() instead of reduce() as array can be empty.
                    // Empty array cannot be reduced.
                    .fold(totalCount, { accumulator, record ->
                        val newCount = accumulator + 1
                        println("Consumed record with key ${record.key()} and value ${record.value()} from ${record.partition()} partition, and updated total count to $newCount")
